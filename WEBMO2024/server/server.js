...
app.post('/api/essen', verifyToken, verifyAdmin, async(req, res) => { //nur admin können diese einträge erstellen mit der entsprechenden token aus middleware die erstellt wird 
    ...
    try {
        ...
        const result = await pool.query(
            'INSERT INTO food (name, price, type) VALUES ($1, $2, $3) RETURNING *', [name, preis, art]
        );
...
async function createNewPlan(wochennummer) {
    ...
    // Überprüfe, ob result.rows existiert und mindestens ein Element enthält
    return result.rows.length > 0 ? result.rows[0].plan_id : null; // Der neue plan_id wird zurückgegeben
}


// Funktion zum Hinzufügen von Essen zum Plan
async function addFoodToPlan(mealPlanId, plan) {
    for (const { tag, essen_id }
        of plan) {
...
app.put('/api/essensplan/:week', async(req, res) => {
    ...
    try {
        ...
        for (const { tag, essen_id }
            of plan) {
            await pool.query(
                'INSERT INTO food_in_plan (plan_id, food_id, day_of_week) VALUES ($1, $2, $3)', [mealPlanId, essen_id, tag]
            );
        }
